...
function jsonFileFromMarkdown(file: string, repo: string, branch: string) {
	return join(
		`src/generated/ast/prose/${repo}_${branch}/`,
...
async function cacheMarkdownAst(
	file: string,
	url: string,
	repo: string,
	branch: string,
...
export async function refreshProse(options: Partial<IEnv> = {}) {
	...
	const docsToUpdate =
		force || currentDocs.length === 0
			...
			: flattenSitemap(sm.sitemap).filter((i) =>
					setChanges.includes(i.filepath),
				);
	...
	return {
		docs,
		changes: sm.hasDeltaInfo ? sm.changes : undefined,
		force,
		cacheFile,
	};
}
