...
export function getEnv(): IEnv {
	try {
		...
		return {
			...
			tsAstPath:
				(import.meta?.env?.TS_AST_PATH as string) ||
				"docs/api/js/js-api.json",

			adminKey:
				import.meta?.env && typeof import.meta?.env === "object"
					? (import.meta?.env[
							`${env.toUpperCase()}_ADMIN_KEY`
						] as string)
					: undefined,
			searchKey:
				import.meta?.env && typeof import.meta?.env === "object"
					? (import.meta?.env[
							`${env.toUpperCase()}_SEARCH_KEY`
						] as string)
					: undefined,
			...
			github_token:
				...
				undefined,
			github_user:
				(import.meta?.env?.GH_USER as string | undefined) || undefined,
			force: import.meta?.env?.FORCE
				? Boolean(import.meta?.env?.FORCE)
				: false,
		};
	} catch (err) {
		console.warn(
