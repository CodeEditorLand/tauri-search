...
(async () => {
	...
	[ApiModel(), RepoModel(), ProseModel(), ConsolidatedModel()].forEach(
		async (model) => {
			if (!idx.includes(model.name)) {
				console.log(
					`- creating the "${model.name}" index with the following config: ${JSON.stringify(
						{
							...model.index,
							stopWords: `${model.index?.stopWords?.length || 0} words`,
						},
					)}`,
				);
	...
	if (errors.length > 0) {
		...
		communicateTaskStatus(ConsolidatedModel(), consolidatedTasks, {
			timeout: 75000,
		});
	}
})();
