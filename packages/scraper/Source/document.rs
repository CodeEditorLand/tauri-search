...
/// A `Selection` captures the key characteristics of a part of the DOM tree
/// that is intersection of an HTML document (`Html`) and a selector
/// (`Selector`).
...
impl Document {
	pub fn new(url:&str) -> Document {
		if url.starts_with("http") {
			Document { url:url.to_string(), data:None }
		} else {
			Document { url:["https://", url].join(""), data:None }
		}
	}
	...
	pub async fn load_document(self) -> Result<LoadedDocument> {
		...
		Ok(LoadedDocument { url:self.url, data:resp })
	}
}
...
impl LoadedDocument {
	/// Parses into a `ParsedDoc` and then adds selectors intended to suit the
	/// `docs.rs` site.
...
pub enum ChildScope {
	...
	All(),
	/// only child selectors with _relative_ path in their `href` property will
	/// be included
	Relative(),
	/// only child selectors with an _absolute_ path in their `href` property
	/// will be included
	Absolute(),
	/// both relative and absolute links to an HTTP resource allowed, other ref
	/// types (e.g., file, Javascript calls, ...) are excluded
	Http(),
	File(),
}
...
impl ParsedDoc {
	...
	/// 3. the "scope" of the href first that defined in call to
	///    `child_selectors`
	...
	pub fn results(&self) -> ParseResults {
		...
		self.selectors.iter().for_each(|(name, _)| {
			match self.get(name) {
				Ok(Some(v)) => {
